...
export default function GameScreen() {
  ...
  const { state, key: playKey } = useLocation() as { state?: LocationState; key: string };
  ...
  const endGame = async () => {
    ...
    /* go to result */
    navigate('/result', {
      state: { score: scoreRef.current, categoryId: currentCatId, difficultyId, elapsedSecs },
    });
  };
  ...
  async function loadQuestion(currentExclude: number[]) {
    ...
    setIsCorrect(false);
    try {
      const question = await getQuestion(currentExclude, categoryId, difficultyId);
      setQ(question);
  ...
  const totalSecs = difficulties.find(d => d.id === difficultyId)?.time_limit_secs ?? 20;
    ...
    <div className="game-screen">
      ...
      <div className="answers-grid mt-6">
        {shuffledOptions.map(opt => {
          ...
          const cls = ['answer-btn', isSel && (isCorrect ? 'correct' : 'wrong')].filter(Boolean).join(' ');
          return (
