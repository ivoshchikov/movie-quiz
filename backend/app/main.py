...
from app.routes import router as quiz_router  # ваши /question и /answer

app = FastAPI(title="Movie Quiz API")

# — CORS (в проде можно ограничить домены)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# ────────────────────────────────────────────────────────────────────────────────
# 1) Admin-API для React-Admin, префикс /admin/api
...
# ────────────────────────────────────────────────────────────────────────────────
# 2) Регистрируем оба роутера
app.include_router(admin_api)   # <— вот он, ваш CRUD для admin
app.include_router(quiz_router) # <— ваш /question и /answer

# 3) Сидим БД при старте (оставляем как есть)
...
# 4) Статика: постеры, админ-UI, квиз
